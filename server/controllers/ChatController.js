 ...
 var chatMAnager = (socket, packtchat) => {
        ...
        socket.on('join', ( request , callback) => {
          ...
          if ( request.type === "medecin" ) {
            ...
            const { error, ticket, medecin, medecinsOnligne, tickets } = UsersController.addPatient({ pseudo: request.pseudo, socket_id: socket.id, name: request.name, id: request.id });
        ...
        socket.on('switch-ticket', ({ selectedUser, type, medecin }, callback) => {
          ...
          var {ticket, tickets, patient, idMedecin} = UsersController.changeTicket({ selectedUser, type, medecin, socket_id: socket.id });
          ...
          if(type === "ready"){
            ...
            packtchat.to(patient.id).emit('ticket-switch', { type ,tickets, ticket });
          }
          else if(type === "fin"){
            packtchat.to(patient.id).emit('ticket-switch', { type ,tickets, ticket });
            ticket= {}
          ...
          packtchat.to(idMedecin).emit('ticket-switch', { type, tickets, ticket: {}  });
