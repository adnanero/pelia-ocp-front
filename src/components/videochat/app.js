...
class ElemenetsCall extends Component {
    ...
    setupPusher(data) {
        ...
        this.channel.bind(`client-feedback-${data.username}`, (msg) =>{
            this.setState({ id_appel: msg.id_appel})
        });
        this.channel.bind(`client-reject-${data.username}`, (msg) =>{
            if(!this.state.isMedecin){
                ...
                .then(res => {
                    //    je doit envoyer l'id au patient res.id
                    this.channel.trigger(`client-feedback-${this.props.match.params.medecin}`, {
                        type: 'id_appel',
                        username: this.state.user.prenom,
                        id_appel: res.data.id
                    })
                });
            }
    ...
    componentDidMount() {
        ...
        try {
            this.notifTone.src = notifTone
        } catch (e) {
            this.notifTone.srcObject =  URL.createObjectURL(notifTone) 
        }
    }
                ...
                {props.callEnd &&
                    ...
                    <Row className="justify-content-center my-5 text-center">
                        <Col>
                            ...
                            <Feedback id_appel={props.id_appel} />            
                        </Col>
            ...
            <Row className="my-5 justify-content-around ">
                <div className= "link-container" style={{width:"70%"}} id="example-collapse-text">
                    <Col >
                        ...
                        <Row className="my-4 generate-passing justify-content-around">
                            <Button onClick={copyCodeToClipboard} className="teleconsultation-button" type="button" aria-label="coupier le lien">
                                Démarrer la téléconsultaion
                            </Button>
                            <Button className="teleconsultation-button mt-2" type="button" aria-label="coupier le lien" onClick={genereId}>Générer un autre lien </Button>
                        </Row>                        
                    </Col>                   
                </div>                
            </Row> 
